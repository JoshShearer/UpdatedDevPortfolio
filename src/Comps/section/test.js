...
describe("Comps_section", () => {
  ...
  // it("should render a '' on the Comps_section and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_section.rename(oneProduct);
  //   renderWithRematchStore(<Comps_section />, store);
  ...
  // it("should reset the Comps_section to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_section.rename(otherProduct);
  //   renderWithRematchStore(<Comps_section />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
