...
describe("Comps_section_faq", () => {
  ...
  // it("should render a '' on the Comps_section_faq and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_section_faq.rename(oneProduct);
  //   renderWithRematchStore(<Comps_section_faq />, store);
  ...
  // it("should reset the Comps_section_faq to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_section_faq.rename(otherProduct);
  //   renderWithRematchStore(<Comps_section_faq />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
