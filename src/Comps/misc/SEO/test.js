...
describe("Comps_misc_SEO", () => {
  ...
  // it("should render a '' on the Comps_misc_SEO and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_misc_SEO.rename(oneProduct);
  //   renderWithRematchStore(<Comps_misc_SEO />, store);
  ...
  // it("should reset the Comps_misc_SEO to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_misc_SEO.rename(otherProduct);
  //   renderWithRematchStore(<Comps_misc_SEO />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
