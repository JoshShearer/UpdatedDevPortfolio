...
describe("Comps_misc_loading", () => {
  ...
  // it("should render a '' on the Comps_misc_loading and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_misc_loading.rename(oneProduct);
  //   renderWithRematchStore(<Comps_misc_loading />, store);
  ...
  // it("should reset the Comps_misc_loading to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_misc_loading.rename(otherProduct);
  //   renderWithRematchStore(<Comps_misc_loading />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
