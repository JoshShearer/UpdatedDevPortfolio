...
describe("Comps_hooks_usePathname", () => {
  ...
  // it("should render a '' on the Comps_hooks_usePathname and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_hooks_usePathname.rename(oneProduct);
  //   renderWithRematchStore(<Comps_hooks_usePathname />, store);
  ...
  // it("should reset the Comps_hooks_usePathname to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_hooks_usePathname.rename(otherProduct);
  //   renderWithRematchStore(<Comps_hooks_usePathname />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
