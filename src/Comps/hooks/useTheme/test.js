...
describe("Comps_hooks_useTheme", () => {
  ...
  // it("should render a '' on the Comps_hooks_useTheme and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_hooks_useTheme.rename(oneProduct);
  //   renderWithRematchStore(<Comps_hooks_useTheme />, store);
  ...
  // it("should reset the Comps_hooks_useTheme to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_hooks_useTheme.rename(otherProduct);
  //   renderWithRematchStore(<Comps_hooks_useTheme />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
