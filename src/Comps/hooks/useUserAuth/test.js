...
describe("Comps_hooks_useUserAuth", () => {
  ...
  // it("should render a '' on the Comps_hooks_useUserAuth and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_hooks_useUserAuth.rename(oneProduct);
  //   renderWithRematchStore(<Comps_hooks_useUserAuth />, store);
  ...
  // it("should reset the Comps_hooks_useUserAuth to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_hooks_useUserAuth.rename(otherProduct);
  //   renderWithRematchStore(<Comps_hooks_useUserAuth />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
