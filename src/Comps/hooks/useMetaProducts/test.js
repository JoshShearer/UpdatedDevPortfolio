...
import { Comps_hooks_useMetaProducts } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_hooks_useMetaProducts", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_hooks_useMetaProducts />, store);
  ...
  // it("should render a '' on the Comps_hooks_useMetaProducts and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_hooks_useMetaProducts.rename(oneProduct);
  //   renderWithRematchStore(<Comps_hooks_useMetaProducts />, store);
  ...
  // it("should reset the Comps_hooks_useMetaProducts to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_hooks_useMetaProducts.rename(otherProduct);
  //   renderWithRematchStore(<Comps_hooks_useMetaProducts />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
