...
describe("Comps_layout_nav_header", () => {
  ...
  // it("should render a '' on the Comps_layout_nav_header and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_nav_header.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_nav_header />, store);
  ...
  // it("should reset the Comps_layout_nav_header to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_nav_header.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_nav_header />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
