...
describe("Comps_layout_nav", () => {
  ...
  // it("should render a '' on the Comps_layout_nav and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_nav.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_nav />, store);
  ...
  // it("should reset the Comps_layout_nav to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_nav.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_nav />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
