...
import { Comps_layout_nav_breadcrumbs } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_layout_nav_breadcrumbs", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_layout_nav_breadcrumbs />, store);
  ...
  // it("should render a '' on the Comps_layout_nav_breadcrumbs and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_nav_breadcrumbs.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_nav_breadcrumbs />, store);
  ...
  // it("should reset the Comps_layout_nav_breadcrumbs to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_nav_breadcrumbs.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_nav_breadcrumbs />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
