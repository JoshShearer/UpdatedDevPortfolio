...
describe("Comps_layout_card", () => {
  ...
  // it("should render a '' on the Comps_layout_card and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_card.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_card />, store);
  ...
  // it("should reset the Comps_layout_card to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_card.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_card />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
