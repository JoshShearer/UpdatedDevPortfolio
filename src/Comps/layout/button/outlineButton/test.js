...
import { Comps_layout_button_outlineButton } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_layout_button_outlineButton", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_layout_button_outlineButton />, store);
  ...
  // it("should render a '' on the Comps_layout_button_outlineButton and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_button_outlineButton.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_button_outlineButton />, store);
  ...
  // it("should reset the Comps_layout_button_outlineButton to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_button_outlineButton.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_button_outlineButton />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
