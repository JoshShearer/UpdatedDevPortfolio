...
import { Comps_layout_button_CTAButton } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_layout_button_CTAButton", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_layout_button_CTAButton />, store);
  ...
  // it("should render a '' on the Comps_layout_button_CTAButton and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_button_CTAButton.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_button_CTAButton />, store);
  ...
  // it("should reset the Comps_layout_button_CTAButton to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_button_CTAButton.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_button_CTAButton />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
