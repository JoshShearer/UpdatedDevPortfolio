...
import { Comps_layout_button_subtleButton } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_layout_button_subtleButton", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_layout_button_subtleButton />, store);
  ...
  // it("should render a '' on the Comps_layout_button_subtleButton and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_button_subtleButton.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_button_subtleButton />, store);
  ...
  // it("should reset the Comps_layout_button_subtleButton to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_button_subtleButton.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_button_subtleButton />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
