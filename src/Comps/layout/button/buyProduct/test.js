...
import { Comps_layout_button_buyProduct } from ".";
import { store, dispatch } from "#src/models/store";

describe("Comps_layout_button_buyProduct", () => {
  beforeEach(() => dispatch({ type: "RESET" })); //clear the state
  // it("should render the 'rename' component", () => {
  //   renderWithRematchStore(<Comps_layout_button_buyProduct />, store);
  ...
  // it("should render a '' on the Comps_layout_button_buyProduct and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_layout_button_buyProduct.rename(oneProduct);
  //   renderWithRematchStore(<Comps_layout_button_buyProduct />, store);
  ...
  // it("should reset the Comps_layout_button_buyProduct to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_layout_button_buyProduct.rename(otherProduct);
  //   renderWithRematchStore(<Comps_layout_button_buyProduct />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
