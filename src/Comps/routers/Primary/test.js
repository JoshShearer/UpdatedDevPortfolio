...
describe("Comps_routers_Primary", () => {
  ...
  // it("should render a '' on the Comps_routers_Primary and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_routers_Primary.rename(oneProduct);
  //   renderWithRematchStore(<Comps_routers_Primary />, store);
  ...
  // it("should reset the Comps_routers_Primary to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_routers_Primary.rename(otherProduct);
  //   renderWithRematchStore(<Comps_routers_Primary />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
