...
describe("Comps_routers_Root", () => {
  ...
  // it("should render a '' on the Comps_routers_Root and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_routers_Root.rename(oneProduct);
  //   renderWithRematchStore(<Comps_routers_Root />, store);
  ...
  // it("should reset the Comps_routers_Root to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_routers_Root.rename(otherProduct);
  //   renderWithRematchStore(<Comps_routers_Root />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
