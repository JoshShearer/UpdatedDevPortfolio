...
describe("Pages_notFound", () => {
  ...
  // it("should render a '' on the Pages_notFound and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Pages_notFound.rename(oneProduct);
  //   renderWithRematchStore(<Pages_notFound />, store);
  ...
  // it("should reset the Pages_notFound to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Pages_notFound.rename(otherProduct);
  //   renderWithRematchStore(<Pages_notFound />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
