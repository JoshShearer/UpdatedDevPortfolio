...
describe("Pages_projectsPage", () => {
  ...
  // it("should render a '' on the Pages_projectsPage and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Pages_projectsPage.rename(oneProduct);
  //   renderWithRematchStore(<Pages_projectsPage />, store);
  ...
  // it("should reset the Pages_projectsPage to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Pages_projectsPage.rename(otherProduct);
  //   renderWithRematchStore(<Pages_projectsPage />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
