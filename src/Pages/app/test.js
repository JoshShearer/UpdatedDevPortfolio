...
describe("Comps_pages_app", () => {
  ...
  // it("should render a '' on the Comps_pages_app and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Comps_pages_app.rename(oneProduct);
  //   renderWithRematchStore(<Comps_pages_app />, store);
  ...
  // it("should reset the Comps_pages_app to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Comps_pages_app.rename(otherProduct);
  //   renderWithRematchStore(<Comps_pages_app />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
