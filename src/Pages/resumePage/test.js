...
describe("Pages_resumePage", () => {
  ...
  // it("should render a '' on the Pages_resumePage and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Pages_resumePage.rename(oneProduct);
  //   renderWithRematchStore(<Pages_resumePage />, store);
  ...
  // it("should reset the Pages_resumePage to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Pages_resumePage.rename(otherProduct);
  //   renderWithRematchStore(<Pages_resumePage />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
