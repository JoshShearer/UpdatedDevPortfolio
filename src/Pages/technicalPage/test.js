...
describe("Pages_technicalPage", () => {
  ...
  // it("should render a '' on the Pages_technicalPage and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Pages_technicalPage.rename(oneProduct);
  //   renderWithRematchStore(<Pages_technicalPage />, store);
  ...
  // it("should reset the Pages_technicalPage to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Pages_technicalPage.rename(otherProduct);
  //   renderWithRematchStore(<Pages_technicalPage />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
