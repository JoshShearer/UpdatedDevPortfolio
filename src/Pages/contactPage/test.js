...
describe("Pages_contactPage", () => {
  ...
  // it("should render a '' on the Pages_contactPage and adjust the ''", async () => {
  //   await dispatch.shop.getProducts();
  //   const [oneThing] = store.getState().rename;
  //   dispatch.Pages_contactPage.rename(oneProduct);
  //   renderWithRematchStore(<Pages_contactPage />, store);
  ...
  // it("should reset the Pages_contactPage to his initial state", async () => {
  //   await dispatch.shop.getProducts();
  //   const [otherProduct] = store.getState().modelData;
  //   dispatch.Pages_contactPage.rename(otherProduct);
  //   renderWithRematchStore(<Pages_contactPage />, store);
  //   expect(await screen.findByRole("list")).toBeInTheDocument();
  //   expect(screen.queryByRole("listitem")).toBeInTheDocument();
  //   expect(screen.getAllByText("$43.00")).toHaveLength(2);
